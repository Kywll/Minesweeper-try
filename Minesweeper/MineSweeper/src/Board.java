...
public class Board {
    ...
    Board(int boardHeight, int boardWidth, int bombLimit){
        ...
        // Gets the size of the board by multiplying the height and width of the board
    ...
    private Tile[] getSurroundingTiles(int startingIndex){
    ...
    public void addBombs(int startingSpot){
        // A list of all the position of the tiles around the starting spot
        Tile[] forbiddenTiles = getSurroundingTiles(startingSpot);
        ...
        // MAKE THE BOARD
        // While loops makes it so that if the bombs placed is not over the limit, then it will loop through again
        while(bombNumber < bombLimit){
            for(int i=0;i<boardHeight;i++){
                for (int j =0;j<boardWidth;j++){
                    ...
                    for (int n=0;n<8;n++){
                        if (board[i][j] == forbiddenTiles[n] && forbiddenTiles[n] != null){
                            surrounding = true;
                            break;
                        }
                    }
                    ...
                    // Check first if the tile is not a bomb already, bomb chance is 1 and bomb limit is not yet reached and the current tile is not clicked and it's not the starting point
                    if (bombChance == 1 && bombNumber < bombLimit && !board[i][j].isClicked && tileIndex != startingSpot && !surrounding && !board[i][j].isBomb()) {
                        // Checks if not a corner
                        if (board[i][j] != board[0][0] && board[i][j] != board[0][boardWidth-1] && board[i][j] != board[boardHeight-1][0] && board[i][j] != board[boardHeight-1][boardWidth-1]){
    ...
    public String winLose(){
        ...
        // Loops and checks if a bomb was clicked and return Lose if yes
        for (int i = 0; i < boardHeight; i++) {
            for (int j = 0; j < boardWidth; j++) {
        ...
        // Counts if all the tiles have been revealed
        int revealTotal = 0;
        for (int i = 0; i < boardHeight; i++) {
            for (int j = 0; j < boardWidth; j++) {
                if (!board[i][j].isBomb() && board[i][j].isClicked) {
        ...
        if (revealTotal >= (boardSize - bombLimit)) {
            revealBombs();
            displayBoard();
        ...
        // Returns none by default if player hasn't won or lose
        return "None";
    }
    ...
    public int checkSurroundingBombs(int row, int column){
